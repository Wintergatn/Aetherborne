...
MeshAllocation::MeshAllocation(const std::shared_ptr<Aetherborne::Buffer>& buffer, Aetherborne::Allocation allocation) {
    this->buffer = buffer;
...
MeshManager::Page::Page(Aetherborne::Engine& engine) : allocator(0, pageSize) {
    ...
    buffer = std::make_shared<Aetherborne::Buffer>(engine, info, allocInfo);
}

MeshManager::MeshManager(Aetherborne::Engine& engine) {
    m_engine = &engine;
}

void MeshManager::setTransferNode(Aetherborne::TransferNode& transferNode) {
...
void MeshManager::createIndexBuffer(std::vector<uint32_t>& indexData) {
    ...
    m_indexBuffer = std::make_shared<Aetherborne::Buffer>(*m_engine, info, allocInfo);
}

MeshAllocation MeshManager::allocateBuffer(size_t size, size_t alignment) {
    //check existing pages
    for (auto& page : m_pages) {
        Aetherborne::Allocation allocation = page->allocator.allocate(size, alignment);
