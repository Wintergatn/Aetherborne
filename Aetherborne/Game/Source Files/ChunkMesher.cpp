...
ChunkMesher::ChunkMesher(Aetherborne::Engine& engine, World& world, BlockManager& blockManager, MeshManager& meshManager) : m_requestQueue(queueSize) {
    m_engine = &engine;
    m_world = &world;
...
void ChunkMesher::setTransferNode(Aetherborne::TransferNode& transferNode) {
    m_transferNode = &transferNode;
}

void ChunkMesher::update(Aetherborne::Clock& clock) {
...
void ChunkMesher::transferMesh(entt::entity entity, size_t index) {
    ...
    if (m_world->registry().has<ChunkMesh>(entity)) {
        ...
        chunkMeshPtr = &m_world->registry().emplace<ChunkMesh>(entity);
    }
