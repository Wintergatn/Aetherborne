...
SelectionBox::SelectionBox(Aetherborne::Engine& engine, Aetherborne::CameraSystem& cameraSystem) {
    m_engine = &engine;
...
void SelectionBox::transfer(Aetherborne::TransferNode& transferNode) {
...
void SelectionBox::createMesh(Aetherborne::TransferNode& transferNode, MeshManager& meshManager) {
    m_mesh = std::make_unique<Aetherborne::Mesh>();
    ...
    std::shared_ptr<Aetherborne::Buffer> vertexBuffer = std::make_shared<Aetherborne::Buffer>(*m_engine, vertexInfo, allocInfo);
    std::shared_ptr<Aetherborne::Buffer> uvsBuffer = std::make_shared<Aetherborne::Buffer>(*m_engine, uvInfo, allocInfo);
...
void SelectionBox::createTexture() {
    ...
    m_image = std::make_unique<Aetherborne::Image>(*m_engine, info, allocInfo);
}
...
void SelectionBox::createPipeline(vk::RenderPass& renderPass) {
    std::vector<char> vertShaderCode = Aetherborne::readFile("shaders/selection.vert.spv");
    std::vector<char> fragShaderCode = Aetherborne::readFile("shaders/selection.frag.spv");

    vk::ShaderModule vertShader = Aetherborne::createShaderModule(m_engine->getGraphics().device(), vertShaderCode);
    vk::ShaderModule fragShader = Aetherborne::createShaderModule(m_engine->getGraphics().device(), fragShaderCode);
