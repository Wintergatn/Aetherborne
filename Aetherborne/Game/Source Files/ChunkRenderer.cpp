...
#include "../../Engine/Header Files/Utilities.h"
#include <glm/glm.hpp>
...
ChunkRenderer::ChunkRenderer(Aetherborne::Engine& engine, Aetherborne::RenderGraph& graph, Aetherborne::AcquireNode& acquireNode, Aetherborne::TransferNode& transferNode, Aetherborne::CameraSystem& cameraSystem, World& world, /*TextureManager& textureManager, SkyboxManager& skyboxManager, SelectionBox& selectionBox,*/ MeshManager& meshManager)
    : Aetherborne::RenderGraph::Node(graph, *engine.getGraphics().graphicsQueue(), vk::PipelineStageFlags::ColorAttachmentOutput) {
    m_engine = &engine;
    ...
    m_uniformBufferUsage = std::make_unique<Aetherborne::RenderGraph::BufferUsage>(*this, vk::AccessFlags::ShaderRead, vk::PipelineStageFlags::VertexShader);
    m_vertexBufferUsage = std::make_unique<Aetherborne::RenderGraph::BufferUsage>(*this, vk::AccessFlags::VertexAttributeRead, vk::PipelineStageFlags::VertexInput);
    m_indexBufferUsage = std::make_unique<Aetherborne::RenderGraph::BufferUsage>(*this, vk::AccessFlags::IndexRead, vk::PipelineStageFlags::VertexInput);
    m_textureUsage = std::make_unique<Aetherborne::RenderGraph::ImageUsage>(*this, vk::ImageLayout::ShaderReadOnlyOptimal, vk::AccessFlags::ShaderRead, vk::PipelineStageFlags::FragmentShader);
    m_skyboxUsage = std::make_unique<Aetherborne::RenderGraph::ImageUsage>(*this, vk::ImageLayout::ShaderReadOnlyOptimal, vk::AccessFlags::ShaderRead, vk::PipelineStageFlags::FragmentShader);
    m_selectionTextureUsage = std::make_unique<Aetherborne::RenderGraph::ImageUsage>(*this, vk::ImageLayout::ShaderReadOnlyOptimal, vk::AccessFlags::ShaderRead, vk::PipelineStageFlags::FragmentShader);
    m_imageUsage = std::make_unique<Aetherborne::RenderGraph::ImageUsage>(*this, vk::ImageLayout::ColorAttachmentOptimal, vk::AccessFlags::ColorAttachmentWrite, vk::PipelineStageFlags::ColorAttachmentOutput);
...
void ChunkRenderer::render(uint32_t currentFrame, vk::CommandBuffer& commandBuffer) {
    ...
    Aetherborne::Frustum frustum = m_cameraSystem->camera().frustum();
...
void ChunkRenderer::createDepthBuffer() {
    ...
    m_depthBuffer = std::make_unique<Aetherborne::Image>(*m_engine, info, allocInfo);
...
void ChunkRenderer::createPipeline() {
    std::vector<char> vertShaderCode = Aetherborne::readFile("shaders/shader.vert.spv");
    std::vector<char> fragShaderCode = Aetherborne::readFile("shaders/shader.frag.spv");

    vk::ShaderModule vertShader = Aetherborne::createShaderModule(m_graphics->device(), vertShaderCode);
    vk::ShaderModule fragShader = Aetherborne::createShaderModule(m_graphics->device(), fragShaderCode);
